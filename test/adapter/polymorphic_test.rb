    ...
    module Adapter
      class PolymorphicTest < ActiveSupport::TestCase
        setup do
          ...
          @attributes_serialization = serializable(@picture) # uses default adapter: attributes
          @json_serialization = serializable(@picture, adapter: :json)
          @json_api_serialization = serializable(@picture, adapter: :json_api)
        end
